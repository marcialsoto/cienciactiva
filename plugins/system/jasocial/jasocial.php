...
class plgSystemJASocial extends JPlugin
{
    ...
    function facebook_like_button($link, $plgParams)
    {
        ...
        $iframe = "<div class=\"ja-fblike-button " . $fb_layout . "\" style=\"width:{$fb_width}; \">{$iframe}</div>";
     ...
     function onAfterRender()
    {
		...
		if(isset($this->_article) && isset($this->_article->fulltext)){
			...
			*/
			if($pos == false && strpos($imageshare, 'http') !== 0 && strpos($imageshare, '//') !== 0){
        ...
        $body = preg_replace_callback($this->_plgCode, array($this, 'genButtons'), $body);
        
        JResponse::setBody($body);
    }
    
    function genButtons($matches) {
